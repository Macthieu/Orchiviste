...
def main() -> int:
    ...
    family_records: dict[str, list[dict[str, Any]]] = {key: [] for key in FAMILY_SPECS}
...
def infer_numero(stem: str) -> str | None:
    match = re.search(r"\b(?:20\d{2}-\d{1,4}|VA1-\d{1,4}|VA-\d{1,4})\b", stem, flags=re.IGNORECASE)
